...
export default function ExamsList() {
    ...
    const filteredExams = useMemo(() => {
        return exams.filter(exam => {
            ...
            // 1. Tab Filter
            if (activeTab !== 'all' && computedStatus !== activeTab) return false;
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.header}>
                <h1 className={styles.pageTitle}>Exams Library</h1>
                <p className={styles.pageSubtitle}>Browse and manage your upcoming and past exams.</p>
            </div>
                    ...
                    paginatedExams.map(exam => (
                        <div key={exam.id} className={styles.card}>
                            ...
                            <div className={styles.cardHeader}>
                                ...
                                <span className={`${styles.statusBadge} ${styles[`status_${getExamStatus(exam)}`]}`}>
                            ...
                            <div className={styles.cardActions}>
                                ...
                                {exam.submission_status === 'submitted' ? (
                                    <div className={styles.scoreWrapper}>
                                        <div className={`${styles.scoreDisplay} ${exam.score !== undefined && exam.score >= 50 ? styles.scorePassed : styles.scoreFailed}`}>
                                            {exam.score ?? '-'}%
                                        </div>
                                    ...
                                    <>
                                        ...
                                        {!exam.submission_status && getExamStatus(exam) === 'ongoing' && (
                                            <button className={styles.primaryBtn} onClick={() => navigate(`/student/exams/${exam.id}/take`)}>
                                                Start Exam
                                            </button>
                                        )}
                                        ...
                                        {!exam.submission_status && getExamStatus(exam) === 'upcoming' && (
                                            <button className={`${styles.primaryBtn} ${styles.primaryBtnDisabled}`} disabled>Start Soon</button>
                                        )}
